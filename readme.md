...
    现在用上了`CH585M`，在家里啃老又过了一年哩，还是啃老舒服呢。

    说正事，原本`CH571F`转到`CH582F`上用了，但是需求又增了，现在为了配合别的外设，得共用同一套文件系统，也为了显示图片，升级到了`CH584F`，随着提出的需求越来越多，我的工作量也随之水涨船高，当真是提出线性升级需求，工作量如指数级增长。

    这次是因为指令解析只带了一个`HandleFn`，面对需要借助指令来区分现在是什么数据传输模式，就不能达到工作预期，只能强行通过外部数据结构将其捆绑在一块（状态机），需要我手动去维护这个指令处理&数据处理的表，这次就需要为它增加不限制数量的数据处理`functions`

修改时间`2026-07-13 08:23`

> 📖 命令解析器已升级至 [cmdTree v2.0](./cmdTree_v2.0.md)（树形路由 + BLE 数据回调自动绑定）
>
> 🍚 新手入门请阅读 [食用手册](./食用手册.md)（环境搭建 + 编译运行测试）
